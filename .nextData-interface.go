...
func main() {
	...
	fmt.Println("now =", nowTime.Format(DateOnlyApi))
	fmt.Println("день недели now", int(nowTime.Weekday()), nowTime.Weekday())

	start := "20250412"

	repeat := "m -2,-1,5,10 4,12"

	USSR, err := parametrParser(nowStr, start, repeat)
	if err != nil {
		fmt.Println("ошибка ввода параметров повториения задачи: ", err)
		return
	}

	dateOut, err := NextDate(USSR)
	if err != nil {
		fmt.Println("ошибка получения даты следующего события: ", err)
		return
	}
